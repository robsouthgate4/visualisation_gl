sim.frag
xtension GL_EXT_draw_buffers : require 
ecision highp float;
efine GLSLIFY 1
rying vec2 vTextureCoord;
iform sampler2D textureVel;
iform sampler2D texturePos;
iform sampler2D textureExtra;
iform sampler2D textureMap;
iform float time;
iform float maxRadius;
iform mat4 uModelMatrix;
iform mat4 uViewMatrix;
iform mat4 uProjectionMatrix;
c3 mod289(vec3 x) { return x - floor(x * (1.0 / 289.0)) * 289.0;  }
c4 mod289(vec4 x) { return x - floor(x * (1.0 / 289.0)) * 289.0;  }
c4 permute(vec4 x) {  return mod289(((x*34.0)+1.0)*x);  }
c4 taylorInvSqrt(vec4 r) {  return 1.79284291400159 - 0.85373472095314 * r;}
oat snoise(vec3 v) { 
